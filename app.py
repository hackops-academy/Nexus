...
# Optional import to prevent crashing on devices missing psutil
try:
    import psutil
...
# --- CONFIGURATION & ARSENAL INTEL ---
MY_TOOLS = [
    {
        "name": "copycat-CLI", 
        "url": "https://github.com/hackops-academy/copycat-CLI", 
        "run_cmd": "cd ~/copycat-CLI && chmod +x copycat-CLI.sh && ./copycat-CLI.sh",
        "manual": "A sophisticated CLI utility for rapid repository cloning, environment setup, and asset duplication. Built with Bash and Python to quickly pull target data or deployment scripts into isolated environments.",
    ...
    },
    {
        ...
        "manual": "A powerful geospatial intelligence and OSINT console. Features coordinate extraction, route mapping, and data visualization. Ensure all intelligence gathering strictly follows the protocol: stay legal. stay anonymous.",
    ...
    },
    {
        ...
        "manual": "Advanced metadata extraction and forensic removal tool. Version 3.0 includes bulk processing capabilities and a streamlined terminal UI to strip EXIF and hidden metadata footprints.",
    ...
    },
    {
        ...
        "manual": "Offensive security utility for generating custom wordlists, bruteforcing basic hashes, and conducting cryptographic vulnerability research. Highly optimized for Termux environments.",
        "use_cases": ["Hash cracking", "Custom dictionary generation", "Cryptographic vulnerability research"]
    },
    {
        "name": "DomainHunter", 
        "url": "https://github.com/hackops-academy/DomainHunter", 
        "run_cmd": "cd ~/DomainHunter && chmod +x DomainHunter.sh && ./DomainHunter.sh",
        "manual": "Subdomain enumeration and reconnaissance framework. Aggregates data from multiple sources to map out external attack surfaces during bug bounty hunting.",
        "use_cases": ["Subdomain discovery", "Attack surface mapping", "Bug bounty reconnaissance"]
    },
    {
        "name": "Ghost-CLI", 
        "url": "https://github.com/hackops-academy/Ghost-CLI", 
        "run_cmd": "cd ~/Ghost-CLI && chmod +x ghost.sh && ./ghost.sh",
        "manual": "Anonymization routing script for Linux and Termux. Manipulates proxy settings and routing to push terminal traffic through secure nodes. Remember: stay legal. stay anonymous.",
    ...
    try:
        ...
        with os.scandir(abs_path) as entries:
            for entry in entries:
                items.append({
                    "name": entry.name, "is_dir": entry.is_dir(), "path": entry.path,
...
def upload_file():
    ...
    path = request.form.get('path', app.config['UPLOAD_FOLDER'])
    if file.filename == '': 
